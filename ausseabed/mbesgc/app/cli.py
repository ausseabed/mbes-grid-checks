...
@click.command()
@click.option("-i", "--input", required=False, help="Path to input QA JSON file")
@click.option(
    "-gf",
    "--grid-file",
    required=False,
...
def cli(input, grid_file):
    ...
    if grid_file is not None:
        if not os.path.isfile(grid_file):
            click.echo("Grid file ({}) does not exist".format(grid_file), err=True)
        ...
        if not os.path.isfile(input):
            click.echo("Input file ({}) does not exist".format(input), err=True)
            sys.exit(os.EX_NOINPUT)
        qajson_folder = os.path.dirname(input)
        qajson = QajsonParser(path=Path(input)).root

    else:
        click.echo("'-input' or '--grid-file' command line arg must be provided")
        sys.exit(os.EX_NOINPUT)
