...
class TestPinkChart(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.rasterised_file = Path(
            "./tests/test_data/pink_chart/pinkchart_outside_raster.tif"
        )
    ...
    def test_ideal_value(self):
        ...
        self.assertAlmostEqual(pc._calc_ideal_value(0.5, -4, -6.3, True), -6.5)

        self.assertAlmostEqual(pc._calc_ideal_value(0.5, -6.3, -4, True), -4.3)

        self.assertAlmostEqual(pc._calc_ideal_value(0.5, 1, -0.1, True), -0.5)

        self.assertAlmostEqual(pc._calc_ideal_value(0.5, 1, 2.1, False), 2.5)

        self.assertAlmostEqual(pc._calc_ideal_value(0.5, 5, 4.1, False), 4.5)

        self.assertAlmostEqual(pc._calc_ideal_value(0.5, 1, 2.2, True), 2.0)
    ...
    def test_calc_ideal_extents(self):
        ...
        ideal_extents = pc._calc_ideal_extents(
            source_res_x,
            source_res_y,
            source_extents,
            target_extents,
